...
| **27** | **Fairness & SLA Guarantees** | **_Planned_: formalize wait-time bounds (percentile, starvation count); prove proactive ≥ FIFO on fairness metrics. THE "PROVE PROACTIVE ≥ FIFO" GOAL IS WITHDRAWN (withdrawal recorded under "Withdrawn claim: proactive >= FIFO on fairness" in `COMPLETION_SUMMARY.md`) and is contradicted on the measure it was about: per-job Gini rises from 0.526 under FIFO to 0.794 under proactive, and max wait from 57.85 to 122.65 ts — materially _less_ fair on both. The picture is not uniform, which is why the honest finding is a trade-off rather than a reversal: run-level Jain is slightly better under proactive (0.924 vs 0.918) and the composite SLA compliance score is higher (0.944 vs 0.933), because those metrics aggregate per run rather than per job. The honest finding is a quantified fairness cost with a partial mitigation: the anti-starvation variant recovers to Gini 0.692. A job counts as starved when its wait exceeds 3× its own runtime.** | **SLA metrics table (`sla_compliance.csv`), `fairness_metrics.csv`. `fairness_formal_analysis.md` and the promised proof sketch are DELETED — no formal proof was ever produced.** | **1–2 days** | **Addresses practical concerns (ops don't want unfair schedulers)** | **Phase 13 (fairness stub)** |
