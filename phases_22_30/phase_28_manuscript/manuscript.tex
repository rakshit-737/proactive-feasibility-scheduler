...
All depend on user runtime estimates, whose inaccuracy is well documented; the
standard model is the $f$-model, $\hat{r} = r \cdot f$ with $f \sim U(1, C)$
\cite{Mualem2001}, together with the observation that over-estimation can
paradoxically \emph{help} backfill \cite{Tsafrir2005}. Feitelson
\cite{Feitelson2005} shows how sensitive backfilling results are to exactly
such evaluation choices. \S\ref{sec:estimates} shows the $f$-model misses a
failure mode that real traces exhibit, because it cannot generate an
under-estimate at all.
