...
\author{
  Rakshit Rameshbabu \\
  \small \textcolor{red}{[AFFILIATION --- fill in before submission]}
}
...
cannot reorder anything. We verify this over $45{,}432$ real dispatch instants
...
{\small Source: \texttt{05\_results/degeneracy/ranking\_degeneracy.csv}}}
...
\caption{Feature variation in ranking decisions: only job-size features differ
across the queue. Of 12 cluster-state features, 7 vary in 0.0\% of instants.}
...
{\small Sources: the two \texttt{*\_equivalence.csv}
files under \texttt{05\_results/}.}}
\label{tab:equiv}
\end{table}
...
7-day windows per trace at offered load $\approx 0.70$. {\small Source: \texttt{trace\_schedulers/trace\_scheduler\_summary.csv}}}
\label{tab:traces}
\end{table}
...
\caption{Twelve policies replayed through two real traces. The Proactive (ML)
and Smallest-first (ML-free) bars are adjacent; every classical policy with a
runtime estimate finishes ahead.}
...
\bottomrule
\end{tabular}
\caption{Real user runtime estimates versus the standard $f$-model. {\small Source: \texttt{trace\_schedulers/trace\_estimate\_quality.csv}}}
\label{tab:est}
\end{table}
...
\bibitem{Feitelson2011} D. G. Feitelson et al., \emph{Parallel Workloads Archive}, \url{https://www.cs.huji.ac.il/labs/parallel/workload/}.
