...
eleven cluster-state features are not useless --- they select \emph{which} table
is used --- but they cannot distinguish two co-queued jobs, which is the only
thing a ranking consumes.
...
preference (for instance, favouring the power-of-two sizes that historically
cleared fastest on that machine). This distinction turns out to matter, and it
is why the equivalence in \S\ref{sec:equivalence} holds on two of the three
substrates rather than all three.
...
On LANL the equivalence fails, and honestly so: the learned table is
non-monotone often enough that \textsc{Proactive} beats a naive size sort by
$14.4\%$. This does not rescue the method. The learned policy is still only a
function of requested size --- it is a \emph{better} size table, not a different
kind of policy --- and as \S\ref{sec:traces} shows, on that same machine it
fails to beat FCFS.
