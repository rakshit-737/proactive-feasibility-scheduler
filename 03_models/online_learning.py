...
<<<<<<< HEAD
    ...
    # slice below would match ZERO rows, silently skipping the drift injection
    # and making the "online retraining recovers from drift" result vacuous.
    stream = df.iloc[split:].reset_index(drop=True).copy()

    drift_n = int(len(stream) * 0.2)
    # position-based slice on the freshly reset index so the first drift_n rows
    # of the stream actually receive the injected distribution shift. The two
    # columns are cast to float first because scaling an int64 column by a
    # non-integer factor would otherwise raise a dtype (LossySetitem) error.
    stream['queue_length'] = stream['queue_length'].astype(float)
    stream['queue_pressure'] = stream['queue_pressure'].astype(float)
    ql = stream.columns.get_loc('queue_length')
